        ...
        if (matchedIndex != -1) {
            if (attribute.toLowerCase().equals("email")) {
                if (this.validateEmail(newValue)) {
                    ...
                    User modifiedUser = new User(modifier.getUserID(), newValue, modifier.getPassword(), modifier.getRole());
                ...
                if (this.validatePassword(newValue)) {
                    User modifiedUser = new User(modifier.getUserID(), modifier.getEmail(), newValue, modifier.getRole());
                    this.database.set(matchedIndex, modifiedUser.toString());
                    updateUsersDatabase();
                    hasBeenModified = true;
                } else {
